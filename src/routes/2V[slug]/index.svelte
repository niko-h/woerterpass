...
{#if task.step === 1 && ((!$showModal || $showStats) || (task.state === 'again' || task.state === 'edited'))}
	<section class="row mt-6" transition:fade>
		<div class="form col col-12 col-lg-6 m-auto py-4 px-3">
			...
			<div class="row mt-5 mb-1">
				<div class="col">
					<button class="btn btn-primary float-right" disabled={inputs['mainVokal'].length < 1} on:click={() => validate()} title="Kontrollieren"><i class="icon-arrow-right icon-large"></i></button>
				</div>
			</div>
		</div>
	</section>
